...
当前稳定版本为 `0.3.1`，**尚未发布到 PyPI**。请从 GitHub 稳定 Tag 安装，不要执行 PyPI 安装命令。
...
稳定通道使用已发布的 `v0.3.1` Tag；`main` 只作为测试通道。升级和回滚只替换 CLI/Gateway 的安装版本，不修改 Agent 配置、设备令牌或本地 Server 配置；完成后需要重启 Agent。
