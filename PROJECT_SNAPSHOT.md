...
- **数据库**: PostgreSQL 18.4 (conda), 库 `mcp_hub`, 用户/密码 `mcp_hub`/`***REMOVED***`
...
```
MCP_HUB_DATABASE_URL=postgresql+asyncpg://mcp_hub:***REMOVED***@localhost:5432/mcp_hub
MCP_HUB_SECRET=REDACTED_OLD_JWT_SECRET
MCP_HUB_GITHUB_CLIENT_ID=REDACTED_OLD_OAUTH_CLIENT_ID
MCP_HUB_GITHUB_CLIENT_SECRET=REDACTED_OLD_OAUTH_CLIENT_SECRET（旧的，已被吊销）
