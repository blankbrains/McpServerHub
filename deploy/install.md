...
设备已接入并产生 Gateway 心跳或真实调用后，打开网页“通知”页即可查看告警。首批规则覆盖：

- Gateway 离线、设备令牌撤销和 Gateway 版本不兼容；
- 同一 Server 连续初始化失败、工具错误率和 P95 延迟；
- 遥测队列持续积压与多设备配置指纹冲突。

相同账户、同一规则和同一设备或 Server 范围内的问题会合并为一条告警，恢复后自动标记为“已恢复”。通知页可按处理中、已恢复和已暂停筛选；每条规则均可暂停或调整阈值。暂停仅关闭该规则的当前告警，不会删除历史通知。

告警只使用设备状态、事件类型、时间、成功状态、耗时、队列深度、版本和配置指纹等脱敏摘要。不会上传或展示工具请求/响应正文、设备令牌、环境变量值、请求头值、完整命令、参数或本地配置内容。Agent 未经过 `mcp-hub serve` 直接连接 Server 时，Hub 没有可用于告警的调用数据。
