...
export default function Login() {
  ...
  // 轮询检测 localStorage 变化（OAuth 回调写入 token）
  const checkAuth = () => {
    const state = getAuthState()
    if (state.token !== auth.token || state.userId !== auth.userId) {
      setAuth(state)
    }
  }
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="text-center space-y-6">
        ...
        <button
          onClick={handleLogin}
          className="inline-flex items-center gap-2 px-6 py-3 bg-gray-900 text-white rounded-xl hover:bg-gray-800 transition-colors text-base font-medium"
        >
