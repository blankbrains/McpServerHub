...
export default function AdminServerDetail() {
  ...
  useEffect(() => {
    if (!serverId) return
    fetch(`/api/v1/admin/servers/${encodeURIComponent(serverId)}`, { headers: { 'x-user-id': uid } })
      .then(r => r.json()).then(r => { if (r.data) setData(r.data) })
      .catch(() => {}).finally(() => setLoading(false))
  }, [serverId])

  if (loading) return <div className="text-center py-16 text-gray-400">加载中...</div>
  if (!data) return <div className="text-center py-16 text-gray-400">Server 不存在</div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-2 items-center">
          ...
          <button onClick={async () => {
            ...
            await fetch(`/api/v1/admin/servers/${encodeURIComponent(server.server_id)}/toggle`, {
          ...
          <select onChange={async (e) => {
            if (!window.confirm(`确定将安全等级设为 ${e.target.value}？`)) return
            await fetch(`/api/v1/admin/servers/${encodeURIComponent(server.server_id)}/security`, {
              method: 'POST', headers: { 'Content-Type': 'application/json', 'x-user-id': uid },
