...
export default function AdminReviews() {
  ...
  const load = () => {
    setLoading(true)
    fetch(`/api/v1/admin/reviews?page=${page}`, { headers: { 'x-user-id': uid } })
      .then(r => r.json()).then(r => { setReviews(r.data || []); setTotal(r.meta?.total || 0) })
  ...
  const deleteReview = async (id: number) => {
    ...
    try {
      const r = await fetch(`/api/v1/admin/reviews/${id}`, { method: 'DELETE', headers: { 'x-user-id': uid } }).then(r => r.json())
