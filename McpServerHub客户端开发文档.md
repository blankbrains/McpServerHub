...
|------|------|
| **版本** | v2.3（SaaS + 自托管双模式） |
| **Python 版本** | ≥ 3.10 (生产环境: conda env `McpServerHub`, Python 3.10) |
| **包管理** | pip + conda (生产) / Uv (本地开发可选) |
| **代码风格** | Ruff + mypy strict |
| **测试框架** | pytest + pytest-asyncio (286 个测试) |
| **CI/CD** | GitHub Actions (`ci.yml` + `deploy.yml`) |
| **数据库** | PostgreSQL 18.4 (生产) / SQLite (quickstart) — SQLAlchemy 2.0 async |
| **部署** | gpu-server (172.19.138.78) / Cloudflare Tunnel |
...
# 方式 B: Conda（生产环境 gpu-server 使用）
conda create -n McpServerHub python=3.10
...
```bash
# 克隆仓库
git clone https://github.com/blankbrains/McpServerHub && cd McpServerHub
...
# 安装 Web Dashboard 依赖
cd src/mcp_hub/web && npm install && npm run build && cd ../../..

# 配置环境变量 (.env)
cat > .env << 'EOF'
MCP_HUB_DATABASE_URL=postgresql+asyncpg://user:password@localhost:5432/mcp_hub
# 或 SQLite quickstart:
# MCP_HUB_DATABASE_URL=sqlite:///./data/servers.db
MCP_HUB_SECRET=your-secret-key-here
MCP_HUB_GITHUB_CLIENT_ID=your-github-client-id
MCP_HUB_GITHUB_CLIENT_SECRET=your-github-client-secret
EOF

# 初始化数据库
python -m mcp_hub.db.migrations

# 导入预置 Server 索引（18 个种子 + 自动发现）
python -m mcp_hub.db.seed

# 验证安装
python -m mcp_hub.cli.app --help
```
...
```bash
# 启动 API 服务器（开发模式 + 热重载）
uvicorn mcp_hub.api.app:app --reload --host 0.0.0.0 --port 3987

# 预期输出
# INFO:     Uvicorn running on http://0.0.0.0:3987
# INFO:     MCP Server Hub API v1.0 启动完成

# 测试基本命令
python -m mcp_hub.cli.app search --category browser
python -m mcp_hub.cli.app info @anthropic/web-search
python -m mcp_hub.cli.app status

# 生产环境启动（gpu-server）
# cd /home/djl/code/McpServerHub
# conda activate McpServerHub
# uvicorn mcp_hub.api.app:app --host 0.0.0.0 --port 3987 --workers 1
```
...
│   ├── search.py        # search / info / compare
│   ├── install.py       # install / uninstall / list
│   ├── manage.py        # start / stop / restart / status
│   ├── logs.py          # logs (--lines / --since / --follow)
│   ├── update.py        # update / rollback / upgrade / version-history
│   ├── config.py        # config sync / export / import
│   ├── daemon.py        # daemon start / stop / status
│   ├── publish.py       # publish / unpublish / my-servers / stats
│   ├── community.py     # rate / review / favorite / favorites
│   ├── trending.py      # trending / top-rated / most-downloaded / new-releases
│   ├── event.py         # event publish / subscribe / list / history
│   ├── auth.py          # login / logout / whoami
│   ├── security.py      # security / security --all
│   ├── token.py         # analyze / analyze --all / optimize
│   ├── monitor.py       # monitor / monitor --all / monitor --watch / reliability
│   ├── create.py        # create (交互式 Server Builder)
│   ├── hub_install.py   # hub-install
│   ├── prompt_install.py # prompt-install
│   ├── registry_sync.py # registry-sync
│   ├── quickstart.py    # quickstart (SQLite 30秒上线)
│   ├── init_cmd.py      # init (初始化配置)
│   └── __init__.py
│
├── api/                 # 18 个路由模块
│   ├── app.py           # FastAPI 实例 + CORS + 中间件 + 路由注册
│   ├── dependencies.py  # FastAPI 认证依赖注入 (get_current_user/get_admin_user)
...
│   ├── routes_auth.py       # /api/v1/auth/* (GitHub OAuth)
│   ├── routes_admin.py      # /api/v1/admin/* (管理后台)
│   ├── routes_realtime.py   # SSE 实时日志
│   ├── routes_config.py     # 配置上传/下载/同步
│   ├── routes_search.py     # 跨 Server 搜索
│   ├── routes_export.py     # 配置导出
│   ├── routes_security.py   # 安全评分 API
│   ├── routes_token.py      # Token 分析 API
│   ├── routes_builder.py    # Server Builder API
│   ├── routes_monitor.py    # 监控大屏聚合 API (dashboard)
│   ├── routes_notifications.py # 通知中心 API
│   ├── routes_presets.py    # 配置方案市场 API
│   ├── routes_usage.py      # 使用统计上报 API
...
│   ├── config_manager.py  # 配置管理 + 用户隔离
│   ├── event_bus.py     # 事件总线 Pub/Sub
│   ├── mcp_gateway.py   # MCP 网关（单 stdio 聚合）
│   ├── auth.py          # GitHub OAuth + 纯 HMAC JWT + CSRF state
│   ├── security_scanner.py # 四维安全评分引擎（含维度最低分机制）
│   ├── token_analyzer.py  # Token 消耗分析 + 优化
│   ├── server_builder.py  # MCP Server Builder (8 模板)
│   ├── monitor.py       # 质量监控 + 可靠性评分
│   ├── local_discovery.py  # 本地 Agent MCP 配置发现 (8 种 Agent)
...
└── web/                 # React 19 SPA
    ...
    └── src/
        ├── App.tsx
        ├── pages/
        │   ├── Dashboard.tsx / Market.tsx / ServerDetail.tsx
        │   ├── MyServers.tsx / ConfigPage.tsx
        │   ├── Builder.tsx / MyConfig.tsx / Login.tsx
        │   ├── MonitorDashboard.tsx / Publish.tsx
        │   ├── ComparePage.tsx / PresetMarket.tsx / ProfilePage.tsx
        │   ├── NotificationsPage.tsx / Guide.tsx / LocalDiscovery.tsx
        │   └── admin/
        │       ├── AdminLayout.tsx / AdminOverview.tsx
        │       ├── AdminUsers.tsx / AdminUserDetail.tsx
        │       ├── AdminServers.tsx / AdminServerDetail.tsx
        │       ├── AdminAnalytics.tsx / AdminReviews.tsx
        │       └── AdminAuditLog.tsx
        └── components/
            ├── StatusBadge.tsx / StarRating.tsx
            ├── LogViewer.tsx / ServerCard.tsx / Layout.tsx
            └── ErrorBoundary.tsx
```
...
### 5.3 数据库操作（SQLAlchemy 2.0 Async）
...
from mcp_hub.db.database import DATABASE_URL, async_session_factory
from mcp_hub.db.models import ServerModel, ReviewModel, UserServerModel


# ✅ 异步查询封装
class ServerRepository:
    """Server 数据访问层 — 所有操作都是 async。"""
    ...
    async def search(
        ...
        stmt = select(ServerModel).where(
            ...
            (ServerModel.description.ilike(f"%{query}%"))
        )
        if category:
            stmt = stmt.where(ServerModel.categories.contains(category))
        stmt = stmt.order_by(ServerModel.download_count.desc()).limit(limit)
        result = await session.execute(stmt)
        return list(result.scalars().all())

    async def get_by_id(self, session: AsyncSession, server_id: str) -> ServerModel | None:
        stmt = select(ServerModel).where(ServerModel.id == server_id)
        result = await session.execute(stmt)
        return result.scalar_one_or_none()


# ✅ 使用 async session context manager
...
async def search_servers(q: str = "", category: str | None = None):
    async with get_session() as session:
        repo = ServerRepository()
        servers = await repo.search(session, q, category)
        return {"success": True, "data": [s.to_dict() for s in servers]}
```

### 5.4 数据库迁移

`init_db()` 启动时自动执行 `_run_migrations()` 处理增量变更（添加列到已有表）：

```python
# db/database.py — _run_migrations()
async with engine.connect() as conn:
    # PostgreSQL: 查 information_schema
    ...
    # SQLite fallback: 查 PRAGMA table_info
...
项目支持 SQLite 和 PostgreSQL 双模式，通过 `MCP_HUB_DATABASE_URL` 自动切换：
...
# SQLAlchemy 引擎自动适配
# sqlite:///./data/servers.db            → aiosqlite
# postgresql+asyncpg://user:pass@host/db → asyncpg

engine = create_async_engine(
    DATABASE_URL,
    echo=False,
    pool_size=5 if "postgresql" in DATABASE_URL else 0,  # SQLite 不用连接池
)
...
tests/
├── __init__.py
├── conftest.py                     # 全局 fixture（db / async_client / cli_runner）
│
├── unit/                           # 单元测试 (8 个模块，145 个测试)
│   ├── test_exceptions.py          # 异常体系
│   ├── test_api_schemas.py         # Pydantic Schema
│   ├── test_health_check.py        # 健康检查
│   ├── test_models.py              # 数据模型
│   ├── test_security_scanner.py    # 🛡️ 安全评分 (33 tests)
│   ├── test_token_analyzer.py      # 📊 Token 分析 (38 tests)
│   ├── test_server_builder.py      # 🛠️ Builder (45 tests)
│   └── test_monitor.py             # 📈 质量监控 (35 tests)
│
├── integration/                    # 集成测试
│   ├── test_core.py                # 核心逻辑集成
│   └── test_init.py                # 初始化流程
...
python -m pytest tests/unit/test_token_analyzer.py -v

# 带覆盖率的测试
python -m pytest --cov=src/mcp_hub --cov-report=html

# 快速失败（开发时）
python -m pytest -x --maxfail=1
...
### 8.1 发布到 PyPI（✅ 已发布: `pip install mcp-hub-cli`）
...
name = "mcp-hub-cli"
version = "1.0.0"
description = "MCP Server Hub — MCP 生态的一站式管理平台"
...
# 本地安装测试
pip install dist/mcp_hub_cli-1.0.0-py3-none-any.whl
...
#### 安全评分引擎（security_scanner.py）

```python
"""四维安全评分引擎。"""

SECURITY_WEIGHTS = {
    "command_safety": 40,     # 安装命令中的危险操作检测
    "package_reputation": 25,  # PyPI/npm 下载量、Stars、维护频率
    "publisher_trust": 20,     # GitHub 账号年龄、是否认证组织
    "code_patterns": 15,       # 静态分析：网络/文件/eval 模式
}
...
#### Token 分析器（token_analyzer.py）

```python
"""Token 消耗分析 + 优化引擎。"""

# 使用 tiktoken cl100k_base 编码器精确计数
# 分析维度: 工具定义 Token / Schema 复杂度 / 同类对比 / 优化建议
# 回退: tiktoken 不可用时使用字符估算
...
#### MCP Server Builder（server_builder.py）

```python
"""交互式 MCP Server 生成器 — 8 个模板。"""

TEMPLATES = {
    "hello":      "最简单的 Hello World MCP Server",
    "echo":       "输入什么返回什么",
    "calculator": "四则运算工具 (add/sub/mul/div)",
    "greet":      "多语言问候工具",
    "weather":    "模拟天气查询 (含城市参数)",
    "memo":       "内存记事本 (CRUD 操作)",
    "search":     "模拟搜索工具 (含分页)",
    "translate":  "模拟翻译工具 (含源/目标语言)",
}

# 输出: Python 或 TypeScript，语法校验通过后打包 ZIP
```
...
# 评分公式:
# score = uptime_24h * 0.40 + uptime_7d * 0.30 + response_score * 0.20 + uptime_1h * 0.10
# 数据来源: health_logs 表 (由 health_check.py 自动写入)
# 输出: mcp monitor (单个) / mcp monitor --all (全部) / mcp monitor --watch (实时)
```
...
### 9.2 数据库（SQLAlchemy 2.0 Async）

```python
# ⚠️ 项目使用 SQLAlchemy 2.0 async，不是 raw sqlite3！
# 所有数据库操作必须是 async/await

# ✅ 正确
async with async_session_factory() as session:
    result = await session.execute(select(ServerModel).where(...))
    servers = result.scalars().all()

# ❌ 错误 — 项目中没有 raw sqlite3 操作
conn = sqlite3.connect(...)  # ❌ 不要这样做

# ⚠️ SQLite 并发写入
# SQLite 默认只允许一个写入者 — 生产环境用 PostgreSQL
# SQLite quickstart 模式启用 WAL:
# SQLAlchemy: connect_args={"check_same_thread": False}
# PRAGMA journal_mode=WAL (通过 SQLAlchemy events 设置)

# ⚠️ PostgreSQL vs SQLite 差异
# - SQLite: sqlite+aiosqlite:///./data/servers.db
# - PostgreSQL: postgresql+asyncpg://user:pass@host:5432/db
# - 通过 config.DATABASE_URL 自动切换
# - PostgreSQL 使用连接池 (pool_size=5)，SQLite 不用
```
...
```bash
# === 本地开发 ===
uv run uvicorn mcp_hub.api.app:app --reload --host 0.0.0.0 --port 3987
...
# === 测试 ===
python -m pytest -x --tb=short      # 快速测试
python -m pytest tests/unit/ -v     # 单元测试
python -m pytest --cov=src/mcp_hub  # 覆盖度

# === 数据库 ===
python -m mcp_hub.db.migrations     # 迁移
python -m mcp_hub.db.seed           # 导入种子数据
...
# === 新功能开发模板 ===
# 1. 写测试: tests/unit/test_xxx.py
# 2. 写代码: src/mcp_hub/core/xxx.py
# 3. 写 CLI: src/mcp_hub/cli/xxx.py
# 4. 写 API: src/mcp_hub/api/routes_xxx.py
# 5. 运行测试确认通过
# 6. 提交: git commit -m "feat(xxx): 描述"
```
...
# 开发完成后
ruff check src/ && mypy src/ && python -m pytest -x
git add -A
git commit -m "feat(xxx): 描述"

# 推送
git push origin feature/xxx
# → GitHub Actions 自动运行 CI (lint + typecheck + test)
```

---

<p align="center">
  <sub>2026 年 7 月 · MCP Server Hub 开发文档 · 面向生产环境</sub>
</p>
